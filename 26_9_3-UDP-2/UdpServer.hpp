...
using Task_t = std::function<std::string(const std::string&, InetAddr&)>;
    ...
    void Start()
    {
        ...
        while(_is_runing)
        {
            ...
            //    std::string peerip = inet_ntoa(peeraddr.sin_addr);
                ...
                //发送消息
                //目标是实现 回调task函数 交给上层执行任务 server只负责通信
                LOG(LogLevel::INFO) << "Calling task function for request from " << peerip << ":" << peerport;
                std::string echo = _task(buffer,peer);
